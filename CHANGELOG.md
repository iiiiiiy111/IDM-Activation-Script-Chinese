...
> 本段记录已合入但还没打 tag 的改动。发版时把标题改成 `## vX.Y.Z — 日期`，
> 并同步 `IAS.cmd` 的 `iasver`（两者不一致会被 `tools/verify-release.ps1` 拦下）。
...
- **`/log=路径` 真正可用**。README、`使用说明.txt`、`ARCHITECTURE.md`、维护清单里都写着 `/log=路径`，但参数解析只认不带路径的 `/log`，用户按文档传路径时日志会被静默写到 `%SystemRoot%\Temp` 而不是指定位置。现在新增 `:parse_logarg` 支持该写法：目录不存在会自动创建，写不进去会提示并回退默认位置（路径仍不能含空格，参数按空格切分）。
